...
const updateGoal = asyncHandler(async (req, res) => {
    ...
    const user = await User.findById(req.user.id) //user id from logged in user.
    ...
    if(goal.user.toString() !== user.id) { // goals have a user field on it which is an object id. Turn it into a string before checking it.
        res.status(401)
